    ...
    def generate_content(self):
        ...
        for b in tube_times:
            ...
            if p not in platforms:
                platforms.append(p)
            tubes.append((b['platformName'], int(b['timeToStation']),
                          str(b['timeToStation'] // 60).rjust(2, ' ') + " min",
                          b['lineName'].replace('Hammersmith',
                                                'Ham').replace('Waterloo',
                                                               'Wloo'),
        ...
        for pnum in platform_order:
            ...
            platform_title = " " + "/".join(sorted(lines_per_platform[pnum]))
            platform_title += " " + platform
            for i, line in enumerate(sorted(lines_per_platform[pnum])):
                self.set_colours(line)
